...
const ReconciliationScreen = ({ route, navigation }) => {
    ...
    useEffect(() => { loadCache(); fetchAllData(); }, [loadCache, fetchAllData]);
    ...
    const processedData = useMemo(() => {
        ...
        const calculatedTenants = Array.isArray(tenantData) ? tenantData.map(t => {
            ...
            return { ...t, units, diff, amount, fixed, transLoss, dgCharge, totalBill, isDgDisabled, isLossDisabled };
        }) : [];

        const totalInputEnergy = Number(gridUnits) + Number(dgUnitsTotal);
        ...
        return { gridUnits, gridAmount, gridFixedPrice, solarUnits, dgUnitsTotal, totalTenantUnitsSum, commonLoss, lossPercent, totalTenantAmountSum, calculatedTenants, profit };
    }, [rawApiData, disabledDgIds, disabledLossIds]);

    const toggleDgCharge = (tenantId) => setDisabledDgIds(prev => prev.includes(tenantId) ? prev.filter(id => id !== tenantId) : [...prev, tenantId]);
    const toggleLossCharge = (tenantId) => setDisabledLossIds(prev => prev.includes(tenantId) ? prev.filter(id => id !== tenantId) : [...prev, tenantId]);
    ...
    return (
        // 1. Root View with Blue background for seamless StatusBar
        <View style={{ flex: 1, backgroundColor: '#333399' }}>
            {/* 2. Professional StatusBar for Blue Header */}
            <StatusBar 
            ...
            <SafeAreaView style={styles.container} edges={['top']}>
                ...
                {/* 4. Content Area with Grey background */}
                <ScrollView
                ...
                <View style={styles.footer}>
                    <TouchableOpacity
                        style={styles.btn}
                        onPress={() => {
                            if (!processedData) return Alert.alert("Error", "Data loading");
...
const styles = StyleSheet.create({
    container: { flex: 1, backgroundColor: '#333399' },
    header: {
        backgroundColor: '#333399',
        ...
        // ❌ paddingTop वाला Platform check हटा दिया गया है
    },
