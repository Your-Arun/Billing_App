...
export default function ForgetScreen({ navigation }) {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      Toast.show({ type: 'error', text1: 'Error', text2: error.response?.data?.msg || 'Failed to send' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResetPassword = async () => {
    if (otp.length < 6 || !newPassword) {
      return Toast.show({ type: 'error', text1: 'Enter 6-digit OTP & Password' });
    }

    setLoading(true);
    try {
      await axios.post(`${API_URL}/reset-password-otp`, { email, otp, newPassword });
      Toast.show({ type: 'success', text1: 'Success ✅', text2: 'Login with new password' });
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.content}>
        <View style={styles.iconCircle}>
          <MaterialCommunityIcons name={step === 1 ? "lock-reset" : "shield-check"} size={50} color="#333399" />
        </View>

        <Text style={styles.title}>{step === 1 ? "Forgot Password?" : "Verify OTP"}</Text>
        <Text style={styles.subTitle}>
          {step === 1 ? "Enter email for 6-digit code" : `Sent to ${email}`}
        </Text>
